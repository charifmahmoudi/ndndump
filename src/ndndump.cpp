...
Ndndump::run()
{
	...
	if (isCSV) {
		...
		if (ofs.bad()) {
			std::cerr << "Error while opening the file " << outputFile << std::endl;
		}else{
			ofs << os.str() ;
		}
	}else{
	...
	try {
		if (block.type() == tlv::Interest) {
			...
			if (matchesFilter(interest.getName())) {
				...
				os  << separator << "INTEREST" << separator << interest.getName();
				//"FreshnessPeriod"  "MustBeFresh"  "Nonce"  "InterestLifetime"
				os << separator << /*"FreshnessPeriod" << */ separator << interest.getMustBeFresh()
						<< separator << interest.getNonce() << separator
						<< interest.getInterestLifetime().count() << std::endl;
				if(isCSV && ofs.good()){
					ofs << os.str();
				}else{
					std::cout << os.str() ;
			...
			if (matchesFilter(data.getName())) {
				...
				os << separator << "DATA" << separator << data.getName();
				//"FreshnessPeriod"  "MustBeFresh"  "Nonce"  "InterestLifetime"
				os << separator << data.getFreshnessPeriod().count() << separator << /*"MustBeFresh" << */ separator
						<< /*"Nonce" << */ separator <</*"InterestLifetime" << */ std::endl;
				if(isCSV && ofs.good()){
					ofs << os.str() ;
				}else{
		...
		std::ostream& os)
{
	...
	case DLT_EN10MB: // pcap encapsulation
	{
		...
		os << inet_ntoa(ipHeader->ip_src) << separator;
